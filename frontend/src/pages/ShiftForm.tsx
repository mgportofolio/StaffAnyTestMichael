...
import { getWeeklyIntervalByDate } from "../helper/functions/weekInterval";
...
const ShiftForm = () => {
  ...
  //       setIsLoading(true);
  //       setErrMsg("");
  //       const dateRange = format(weekRange.firstRange!, "yyyy-MM-dd") + "|" + format(weekRange.lastRange!, "yyyy-MM-dd");
  //       const { results } = await getShiftsByWeekRange(dateRange);
  //       setRows(results);
  //     } catch (error) {
  //       const message = getErrorMessage(error);
  //       setErrMsg(message);
  ...
  const onSubmit = handleSubmit(async ({ name, date, startTime, endTime }) => {
    try {
      ...
      const payload = {
        ...
        intervalWeek: getWeeklyIntervalByDate(date!)
      };
      console.log(payload);
      if (id) {
